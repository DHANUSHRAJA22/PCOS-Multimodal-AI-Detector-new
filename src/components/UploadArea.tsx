...
interface UploadAreaProps {
  ...
  /** NEW: let parent push in an image (e.g. from SampleImages) */
...
export function UploadArea({
  ...
  const [processedImage, setProcessedImage] = useState<ProcessedImage | null>(value)
  const [isDragOver, setIsDragOver] = useState(false)
  const [isProcessing, setIsProcessing] = useState(false)
  const [showCamera, setShowCamera] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)

  // Keep internal preview synced with parent-controlled value
  useEffect(() => {
    setProcessedImage(value ?? null)
  }, [value])
  ...
  return (
    <div className={cn('space-y-4', className)}>
            ...
            <div className="text-center space-y-4">
              {isProcessing ? (
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  ...
                  <div>
                    <p className="text-sm font-medium mb-2">Click to upload or drag & drop</p>
      ...
      <CameraCapture open={showCamera} onOpenChange={setShowCamera} onCapture={handleCameraCapture} />
    </div>
  )
}
